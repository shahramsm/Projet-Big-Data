...
def simpleKmeans(data, nb_clusters):
    ...
    #############################
    # Select initial centroides for kmeanPP#
    #############################
    ...
    while not clusteringDone:
        ...
        if switch == 0 or number_of_steps == 100:
            ...
            centroids = sc.parallelize(centroidesCluster.collect())
            prev_assignment = min_dist
            number_of_steps += 1

    print("Elapsed time: " + str(datetime.datetime.now() - startTime))
    print("NofS: " + str(number_of_steps))
...
if __name__ == "__main__":
    ...
    lines = sc.textFile("hdfs:/user/user16/data/iris.data.txt")
    ...
    clustering[0].sortByKey().coalesce(1).saveAsTextFile("hdfs:/user/user16/output_final")
